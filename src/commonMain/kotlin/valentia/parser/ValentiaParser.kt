...
open class ValentiaParser(
    ...
    companion object {
        ...
        fun type(@Language("kotlin", prefix = "typealias Example = ", suffix = "") str: String): Type =
...
// SECTION: classMembers
    ...
    fun optionalTypeWithColon(): Type? {
        return if (expectOpt(":")) {
            NLs(); type()
        } else null
    }
    ...
    fun functionType(): FuncType? {
        ...
        return FuncType(ret, typeParams, receiver)
    }

    //functionTypeParameters
    //    : LPAREN NL* (parameter | type)? (NL* COMMA NL* (parameter | type))* (NL* COMMA)? NL* RPAREN
    //    ;
    fun functionTypeParameters(): List<FuncType.Item>? {
        return expectAndRecover("(", ")", nullIfNotMatching = true) {
            NLs()
            val first = ORNullable(
                { parameter()?.let { FuncType.Item(it) } },
                { type()?.let { FuncType.Item(it) } }
            )
            val rest = zeroOrMore {
    ...
    //    ;
    fun receiverType(): Type? {
        val spos = pos
        val modifiers = zeroOrMore { typeModifier() }
        return ORNullable({ parenthesizedType() }, { nullableType() }, { typeReference() })?.withModifiers(modifiers)
            ?: return null.also { pos = spos }
    ...
    //    ;
    fun parenthesizedUserType(): Type? {
        return expectAndRecover("(", ")", nullIfNotMatching = true) {
            NLs()
...
// SECTION: expressions
    ...
    //    ;
    fun disjunction(state: ExpressionState? = null): Expr? =
        binop({ expectAnyOpt("||") }, initialNLs = true) { conjunction(state) }

    //conjunction
    //    : equality (NL* CONJ NL* equality)*
    //    ;
    fun conjunction(state: ExpressionState? = null): Expr? =
        binop({ expectAnyOpt("&&") }, initialNLs = true) { equality(state) }

    //equality
    //    : comparison (equalityOperator NL* comparison)*
    //    ;
    fun equality(state: ExpressionState? = null): Expr? =
        binop({ expectAnyOpt(EQUALITY_OPERATOR) }, initialNLs = false) { comparison(state) }

    //comparison
    //    : genericCallLikeComparison (comparisonOperator NL* genericCallLikeComparison)*
    //    ;
    fun comparison(state: ExpressionState? = null): Expr? =
        binop({ expectAnyOpt(COMPARISON_OPERATOR) }, initialNLs = false) { genericCallLikeComparison(state) }
    ...
    //    ;
    fun elvisExpression(state: ExpressionState? = null): Expr? =
        binop({ expectAnyOpt("?:") }, initialNLs = true) { infixFunctionCall(state) }
    ...
    private fun <T> parseListWithStartEnd(
        start: String,
        end: String,
    ...
    fun anonymousFunction(): AnonymousFunctionExpr? {
        ...
        return AnonymousFunctionExpr(
            FunDecl(
                "",
                paramsWithOptType.map { FuncValueParam(it.id, it.type ?: UnknownType) },
                retType,
                receiver = receiver,
                where = typeConstraints,
                body = body,
                modifiers = if (isSuspend) Modifiers(FunctionModifier.SUSPEND) else Modifiers.EMPTY
            )
        )
    }
    ...
    fun whenCondition(): WhenExpr.ConditionBase {
        return when {
            matches("in") || matches("!in") -> {
                WhenExpr.ConditionIn(
                    op = inIsOperator().also { NLs() },
                    expr = expression() ?: error("Expression expected in/ $this")
                )
            }

            matches("is") || matches("!is") -> {
                WhenExpr.ConditionIs(
                    op = inIsOperator().also { NLs() },
                    type = type() ?: error("Expression expected is $this")
                )
            }
    ...
    fun callableReference(): Expr? {
        ...
        NLs()
        val kind = if (matches("class")) {
            CLASS(); "class"
        } else simpleIdentifier()
    ...
    //    ;
    fun typeParameterModifiers(atLeastOne: Boolean = true): Modifiers =
        Modifiers(multiple(atLeastOne = atLeastOne) { typeParameterModifier() })
...
// SECTION: inside
    ...
    companion object {
        ...
        val ANNOTATION_SITES =
            setOf("field", "property", "get", "set", "receiver", "param", "setparam", "delegate", "file")
        ...
        val ALL_MODIFIERS: Map<String, Modifier> =
            CLASS_MODIFIERS + MEMBER_MODIFIERS + VISIBILITY_MODIFIERS + FUNCTION_MODIFIERS + PROPERTY_MODIFIERS + INHERITANCE_MODIFIERS + PARAMETER_MODIFIERS + PLATFORM_MODIFIERS
    }
...
interface BaseTokenReader : BaseConsumer {
    ...
    override fun EOF(message: String?) = check(eof) { "Not EOF found but ${peek()} message=$message" }
    ...
    fun expectAny(strs: Set<String>): String = expectAnyOpt(strs) ?: error("Couldn't find ${strs} in $this")
    fun <T> expectAnyOpt(strs: Map<String, T>): T? = strs[peek().str]?.also { skip() }
    fun <T> expectAny(strs: Map<String, T>): T = expectAnyOpt(strs) ?: error("Couldn't find ${strs} in $this")
...
inline fun BaseConsumer.debug(message: () -> String) = Unit
...
interface BaseConsumer {
    ...
    fun reportError(e: Throwable) = Unit
    ...
    fun expectAny(vararg strs: String): String =
        expectAnyOpt(*strs) ?: error("Couldn't find ${strs.toList()} in $this")

    fun expectAny(str1: String, consume: Boolean = true): String = when {
        matches(str1, consume = consume) -> str1
        else -> error("Couldn't find $str1 in $this")
    }

    fun expectAny(str1: String, str2: String, consume: Boolean = true): String = when {
        matches(str1, consume = consume) -> str1
        matches(str2, consume = consume) -> str2
        else -> error("Couldn't find $str1 or $str2 in $this")
    }

    fun expectAny(str1: String, str2: String, str3: String, consume: Boolean = true): String = when {
        matches(str1, consume = consume) -> str1
        matches(str2, consume = consume) -> str2
        matches(str3, consume = consume) -> str3
        else -> error("Couldn't find $str1, $str2 or $str3 in $this")
    }

    fun expectAny(str1: String, str2: String, str3: String, str4: String, consume: Boolean = true): String = when {
        matches(str1, consume = consume) -> str1
        matches(str2, consume = consume) -> str2
        matches(str3, consume = consume) -> str3
        matches(str4, consume = consume) -> str4
        else -> error("Couldn't find $str1, $str2, $str3 or $str4 in $this")
    }
    ...
    fun expectOpt(str: String): Boolean = matches(str, consume = true)

    fun EOF(message: String? = null) = check(eof) { "Not EOF found (message=$message)" }
...
inline fun <T> BaseConsumer.OR(vararg funcs: () -> T?, name: String? = null): T =
    ORNullable(*funcs, name = name) ?: error("  Couldn't match any of OR[$name][$this] [${funcs.size}]")
...
inline fun <T> BaseConsumer.multiple(atLeastOne: Boolean, catch: Boolean = true, block: () -> T?): List<T> {
    ...
    loop@while (hasMore) {
        if (out.size >= 50_000) TODO("Too long")
        val oldPos = pos
        val res = block()
        if (res == null) {
            pos = oldPos
